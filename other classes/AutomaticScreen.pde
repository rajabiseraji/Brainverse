...
public class AutomaticScreen extends AppScreen{
    Torus torus;
    Moon moon;
    Ocean ocean;
    Star star;
    ...
    void generateData() {
        for (Wave w : waves) {
            w.waveValue = (int)noise(0.002) * 100;
            switch (w.waveName) {
                case "alpha" :
                    alphaSlider.setSliderValue(w.waveValue);
                break;	
                case "beta" :
                    betaSlider.setSliderValue(w.waveValue);
                break;	
                case "gamma" :
                    gammaSlider.setSliderValue(w.waveValue);
                break;	
                case "tetha" :
                    tethaSlider.setSliderValue(w.waveValue);
                break;	
                case "delta" :
                    deltaSlider.setSliderValue(w.waveValue);
                break;	
            }
            println("w.waveValue: "+w.waveValue);
        }
    }
